...
#+BEGIN_SRC emacs-lisp :tangle yes
  (progn
    (use-package-ensure-elpa 'password-store 'f 'nil :ensure)
    (eval-and-compile
      (add-to-list 'load-path "/home/jethro/.emacs.d/elisp"))
    (let
	((now
	  (current-time)))
      (message "%s..." "Loading package password-store")
      (prog1
	  (if
	      (not
	      ...
	      (ignore
	       (message
		(format "Cannot load %s" 'password-store)))
	    (let
		((now
		  (current-time)))
	      (message "%s..." "Configuring package password-store")
	      (prog1
		  (progn
			...
			(display-warning 'use-package
					 (format "%s %s: %s" "password-store" ":config"
		...
		(let
		  ...
		  (if
		      (> elapsed 0.1)
		      (message "%s...done (%.3fs)" "Configuring package password-store" elapsed)
		    (message "%s...done" "Configuring package password-store"))))))
	(let
	    ((elapsed
	      (float-time
	       (time-subtract
	  ...
	  (if
	      (> elapsed 0.1)
	      (message "%s...done (%.3fs)" "Loading package password-store" elapsed)
	    (message "%s...done" "Loading package password-store"))))))
#+END_SRC
* Core Layer
** User Configuration
   ...
   (setq org-capture-templates
      ...
      Captured %<%Y-%m-%d %H:%M>")
           ("w" "Web site" entry (file "~/.org/deft/websites.org")
            "* %c\n" :immediate-finish t)
           ("p" "paper" entry (file+headline "~/.org/gtd/projects.org" "Reading")
            "* TODO Read: %(jethro/trim-citation-title \"%:title\") :@home:@school: \n%a" :immediate-finish t)))
 #+END_SRC
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (setq org-publish-project-alist
        '(("org-notes"
           ...
           :base-extension "org"
           :publishing-directory "~/.org/deft/html/"
           :recursive t
           :publishing-function org-html-publish-to-html
           :headline-levels 4
           :auto-sitemap t 
           :html-preamble (concat "<link rel='stylesheet' type='text/css' href='http://www.pirilampo.org/styles/readtheorg/css/htmlize.css'/>"
